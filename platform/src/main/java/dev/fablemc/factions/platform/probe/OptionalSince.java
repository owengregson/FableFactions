...
/**
 * A manifest entry whose handle is EXPECTED absent below a declared version: its
 * absence is a typed, quiet outcome with a fallback declared right at the entry —
 * never a mapping break, never a bare {@code @Nullable} at a call site (Mental Tier-2,
 * CONTRACTS §3). A server at or above {@link #since()} that still fails to resolve is a
 * mapping break the resolver's own probe loud-logs; that is the resolver's job.
 *
 * <p>The declared fallback is what a caller uses when the handle is absent: a same-typed
 * value for flag entries (e.g. {@code false} for a capability flag), or {@code null} with
 * a {@link #fallbackNote() note} for handle entries whose caller substitutes an era value
 * of its own. Either way the fallback is DECLARED, not a magic literal at the read site.
 *
...
public final class OptionalSince<T> implements ManifestEntry {
    ...
    /** The version the handle first appears, e.g. {@code 1.14}. Never blank. */
