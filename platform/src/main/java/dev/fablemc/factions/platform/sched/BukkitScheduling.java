...
/**
 * Classic single-threaded scheduling: every non-Folia server, 1.7.10 through
 * current. "Global", "at location" and "on entity" all collapse onto the main
 * thread; entity targeting still honours the retired callback so callers get
 * identical semantics on both platforms.
 *
 * <p>Owning thread(s): callable from any thread (delegates to {@code BukkitScheduler}).
 * Mutability class: immutable (holds only the plugin handle).
 */
public final class BukkitScheduling implements Scheduling {
    ...
    public boolean isOwnedByCurrentRegion(@NotNull Entity entity) {
        // One region: the main thread owns every entity. Callers reach this only from
        // the owning thread, so the reads they are gating are region-correct.
        return true;
    }
    ...
    /** Cancellation over a live {@code BukkitTask}. */
    private static final class BukkitHandle implements TaskHandle {

        private final BukkitTask task;

        BukkitHandle(@NotNull BukkitTask task) {
            this.task = task;
        }
